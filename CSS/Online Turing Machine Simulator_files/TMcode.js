	...
	for(i = 0; i < lines.length && !error ; i += comment_lines + 1){
		...
		if(original_line.indexOf('//') == 0 || original_line.length == 0){
			...
			if(line.indexOf('init:') == 0){
				...
				machine_name = escapeHTML(lines[i].split(':')[1].replace(/^\s+|\s+$/g, ''));
				comment_lines = 0;
			}
							...
							if(consequent.length != 2 * new_ntapes + 1){
								...
								for (var j = 1; j < new_ntapes + 1; j++) {
									if(consequent[j].length != 1){
										error = true;
										errmsg = "Incorrect element in line " + (i + comment_lines + 1) + ": symbols must be single characters";
						...
						if(!error){
							for(var j = 1; j < new_ntapes+1 && !error; j++){
								if(consequent[new_ntapes + j] != '<' && consequent[new_ntapes+j] != '>' && consequent[new_ntapes+j] != '-'){
									error = true;
									errmsg = "Incorrect transition in line " + (i+comment_lines+1) + ". Movements must be left (<), right (>) or center (-).";
								}
							}
						}	
					}
...
function transicion(){
	...
	//Otherwise verify if the input is accepted.
		...
		for(var j = 0; j < n_tapes; j++){
			output_text += "Tape " + (j + 1) + ": " + escapeHTML(tapes[j].get_string()) + "<br>";
		}
...
// function toggleAdvancedExamples(toAdv){

// 	if(toAdv){
// 		$("#examplesLink").removeAttribute("linkEnabled");
// 		$("#examplesLink").className = "linkDisabled";
// 		$("#advancedLink").removeAttribute("linkDisabled");
// 		$("#advancedLink").className = "linkEnabled";
// 		$('#examplesDiv').fadeOut(250, function(){ 
// 			$('#advancedDiv').fadeIn(250); 
// 		});
// 	}

// 	else{
// 		$("#examplesLink").removeAttribute("linkDisabled");
// 		$("#examplesLink").className = "linkEnabled";
// 		$("#advancedLink").removeAttribute("linkEnabled");
// 		$("#advancedLink").className = "linkDisabled";
// 		$('#advancedDiv').fadeOut(250, function(){
// 			$('#examplesDiv').fadeIn(250);
// 		});
// 	}
// }